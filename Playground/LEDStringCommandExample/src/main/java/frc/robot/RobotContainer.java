...
public class RobotContainer {
  ...
  public void debugScheduleDefaultCommands() {
    // Create the time base lambda up front
    DoubleSupplier timeSupplier = () -> m_currentLoopTime;
    ...
    var fadeInGroup = new ParallelCommandGroup();
    for (LEDSubstringSubsystem substring : m_gerryRigLEDStrings) {
      fadeInGroup.addCommands(new LEDStringFadeInCommand(Color.kRed, 0.5, timeSupplier, substring));
    }
    commandGroup.addCommands(fadeInGroup);
    ...
    var reverseFadeInGroup = new ParallelCommandGroup();
    for (LEDSubstringSubsystem substring : m_gerryRigLEDStrings) {
      var command = new LEDStringFadeInCommand(Color.kGreen, 0.5, timeSupplier, substring);
      command.isReversed = true;
      reverseFadeInGroup.addCommands(command);
    }
    commandGroup.addCommands(reverseFadeInGroup);
    ...
    // Create a parallel group. This will wait until all strings have finished animating.
    var fadeInGroup2 = new ParallelCommandGroup();
    for (LEDSubstringSubsystem substring : m_gerryRigLEDStrings) {
      var command = new LEDStringFadeInCommand(Color.kBlue, 0.5, timeSupplier, substring);
      command.isReversed = false;
      fadeInGroup2.addCommands(command);
    }
    commandGroup.addCommands(fadeInGroup2);
    ...
    var reverseFadeInGroup2 = new ParallelCommandGroup();
    for (LEDSubstringSubsystem substring : m_gerryRigLEDStrings) {
      var command = new LEDStringFadeInCommand(Color.kWhite, 0.5, timeSupplier, substring);
      command.isReversed = true;
      reverseFadeInGroup2.addCommands(command);
    }
    commandGroup.addCommands(reverseFadeInGroup2);
